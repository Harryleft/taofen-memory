...
// 布局模式配置
export type HeaderLayout = 'standard' | 'hero';
...
// 标准布局组件
function StandardLayout({ config, accentColorClass, accentBgColorClass, accentColorValue }: { 
  config: HeaderConfig; 
  accentColorClass: string;
  accentBgColorClass: string;
  accentColorValue: string;
}) {
  const { title, subtitle, description, icon, culturalElement, decorativeElement } = config;
...
const BaseHeader: React.FC<BaseHeaderProps> = ({ config }) => {
  ...
  const backgroundColorClass = backgroundColorVariants[backgroundColor] || 'bg-white';
  ...
  const headerClassName = useMemo(() => {
    const baseClasses = layout === 'hero' 
      ? 'fixed top-0 left-0 right-0 z-50 transition-all duration-700'
      : 'minimal-header text-center p-8 md:p-12';
    
    return `${baseClasses} ${backgroundColorClass}`;
  }, [layout, backgroundColorClass]);
  ...
  return (
    <>
      <header className={headerClassName}>
        {layout === 'hero' ? (
          ...
          <StandardLayout 
            config={config} 
            accentColorClass={accentColorClass}
            accentBgColorClass={accentBgColorClass}
      ...
      {/* 移动端菜单 */}
      {layout === 'hero' && navigation && showMobileMenu && (
        <MobileMenu
          isOpen={isMenuOpen}
          items={navigation}
          onClose={closeMenu}
